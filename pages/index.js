...
// ============================================================
function FormCTACard({ formUrl, prefill }) {
  const hasPhone = prefill?.telephone;
  return (
    <div className="form-cta-card">
      <div className="form-cta-icon">{hasPhone ? '✅' : '👨‍🔧'}</div>
      <h3 className="form-cta-title">{hasPhone ? 'Numéro enregistré' : 'Passez à l\'étape suivante'}</h3>
      <p className="form-cta-text">
        {hasPhone ? (
          <>Ton numéro <strong>{prefill.telephone}</strong> est pré-rempli. Clique pour confirmer et un expert te rappelle.</>
        ) : (
...
export default function Home() {
  ...
  const sendMessage = async (messageText) => {
    ...
    try {
      ...
      // HANDLE ACTION : Afficher carte CTA après délai + propager UTM/fbclid + prefill tél
      if (data.action?.type === "OPEN_FORM" && data.action?.url) {
        var tracking = getTrackingParams();
        var url = data.action.url;
        var prefill = data.action?.prefill || null;
        // Ajouter tel dans l'URL si pré-rempli
        if (prefill?.telephone) {
          var telParam = 'tel=' + encodeURIComponent(prefill.telephone);
          var hashIdx0 = url.indexOf('#');
          if (hashIdx0 !== -1) {
            url = url.substring(0, hashIdx0) + (url.substring(0, hashIdx0).indexOf('?') !== -1 ? '&' : '?') + telParam + url.substring(hashIdx0);
          } else {
            url = url + (url.indexOf('?') !== -1 ? '&' : '?') + telParam;
          }
        }
        // Ajouter tracking UTM
        if (tracking) {
        ...
        <main className="chat-messages">
          ...
          {showFormCTA && (
            <div className="message message-assistant cta-message">
              <div className="avatar">🔧</div>
              <FormCTACard formUrl={formUrl} prefill={formPrefill} />
            </div>
          )}
