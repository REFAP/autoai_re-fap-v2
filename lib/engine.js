...
import {
  ...
  sanitizeReplyNonFAP, ensureHeadingNewlines, stripSingleItalics
} from './postprocess';
...
export async function handleChat({ question, historique }) {
  ...
  reply = ensureHeadingNewlines(reply); // titres sur leur ligne
  reply = stripSingleItalics(reply);    // enlève *italiques* parasites
  ...
  if (category === 'FAP') {
    if (!triage) {
      ...
      // solution FAP : bénéfices + Oui/Non
