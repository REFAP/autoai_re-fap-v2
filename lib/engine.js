...
export async function handleChat({ question, historique }) {
  ...
  // 1) CatÃ©gorie minimisÃ©e (FAP vs DIAG, jamais OOD)
  ...
  // 6) CTAs inline (affichÃ©s sous la bulle cÃ´tÃ© UI)
...
=======
ï»¿import { detectCategory, needsTriage } from "./detect";
import { buildSystemPrompt } from "./prompt";
import { fallbackTriage, fallbackAnswer } from "./fallbacks"; // âŒ pas de fallbackOOD
...
export async function handleChat({ question, historique }) {
  ...
  // 1) FAP vs DIAG uniquement (on neutralise toute idÃ©e d'OOD)
