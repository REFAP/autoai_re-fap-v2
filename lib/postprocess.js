...
=======
﻿// lib/postprocess.js – nettoyage/normalisation Markdown (compact, lisible).

// 1) Saut de ligne après chaque titre (## ou ###) s’il manque.
function ensureHeadingNewlines(text) {
  const t = String(text || '');
  return t.replace(/^(#{2,3}\s+[^\n]+)\s+(?!\n)/gm, '$1\n');
}
...
export default function postProcess(text, _category = 'GEN') {
  ...
  if (!out) {
    out = "Désolé, je n’ai pas pu formuler une réponse. Réessaie en précisant les symptômes (voyant, fumée, perte de puissance, etc.).";
  }
  return out;
}
>>>>>>> Stashed changes
